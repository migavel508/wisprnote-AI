...
async function upsertItem(userId: string, workspaceId: string, it: KnowledgeItemInput): Promise<void> {
  await query(
    `INSERT INTO knowledge_item (user_id, workspace_id, source, source_id, type, title, body, people, links, raw, occurred_at, synced_at)
    ...
    [
      userId, workspaceId, it.source, it.source_id, it.type, it.title ?? null, it.body ?? null,
...
export async function runConnectorSync(): Promise<ConnectorSyncResult> {
  ...
  const creds = await query<{ user_id: string; workspace_id: string; source: string }>(
    `SELECT user_id, workspace_id, source FROM connector_credentials ORDER BY updated_at ASC LIMIT $1`,
    [SOURCE_CAP],
  );

 ...
 *   • status  — one per Jira ticket thread ("what is the status of PROJ-14?") → expect jira:KEY
...
export async function paraphraseGoldenSet(spaceId: string, cap = 60): Promise<{ added: number; total: number }> {
  ...
  for (const g of src.slice(0, cap)) {
    ...
    const p = await flash(
      'Rewrite the question as a natural question a busy teammate would ask about the SAME underlying subject. Rules: do NOT reuse ticket keys/IDs (e.g. PROJ-14), do NOT copy exact phrases from the question — refer to the topic in your own words. Output ONLY the rewritten question, one line.',

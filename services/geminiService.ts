...
/**
 * Process an audio file directly using Gemini's multi-modal capabilities.
 * Specifically optimized for Long Audio and Multilingual (Tamil/English) inputs.
 */
export const processAudioFile = async (base64Data: string, mimeType: string): Promise<{ 
  ...
  const response = await ai.models.generateContent({
    ...
    contents: [
      {
        ...
        parts: [
          ...
          {
            text: `Carefully process this entire audio recording. 
            
            MULTILINGUAL INSTRUCTION:
            The audio contains a mix of English and Tamil. 
            - DO NOT ignore or skip the Tamil sections. 
            - Transcribe exactly what is heard in BOTH languages.
            
            LONG AUDIO CONTEXT:
            - This is a long file. Do not truncate your analysis. 
            - Identify all key discussion points from start to finish.
            
            REQUIRED OUTPUT (JSON):
            1. 'transcription': Verbatim text capturing the conversation in its original languages (Tamil/English).
            2. 'summary': A detailed, professional summary in English that synthesizes points from both languages.
            3. 'todos': A prioritized list of tasks (in English) extracted from the entire conversation.
            
            Return result ONLY as a JSON object.`
          }
        ]
      }
    ],
    config: {
      ...
      responseSchema: {
        ...
        required: ["transcription", "summary", "todos"]
      }
    }
  });
  ...
  try {
    ...
    throw new Error("Neural processing failed. Please ensure the file is a valid audio format.");
  }
};
...
export const autoCorrectAndRestructure = async (content: string): Promise<string> => {
  ...
  const response = await ai.models.generateContent({
    ...
    2. Remove all repetitions.
    3. Correct grammar and punctuation.
    4. Restructure into logical paragraphs or bullet points.
    5. Support and maintain both Tamil and English words if present.
...
export const suggestUnifiedMetadata = async (content: string): Promise<{ 
  ...
  const response = await ai.models.generateContent({
    model: 'gemini-3-flash-preview',
    contents: `Analyze this content. Suggest: Title (5 words max), 3 tags, Category, Vibe color, Priority, Complexity (1, 2, 3, 5, 8), Theme. Return ONLY JSON:`,
    config: { 
      responseMimeType: "application/json",
      responseSchema: {
        type: Type.OBJECT,

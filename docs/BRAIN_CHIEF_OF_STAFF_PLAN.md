...
- **Guesses are counted as evidence.** A thread's "7 linked commits" counts *any* edge, including
  semantic guesses — so the ledger's own facts ("someone started coding PROJ-14") can be false.
...
Chat's `brain_threads` returned **PROJ-14 and PROJ-13 twice** (one copy with evidence, one with
...
Today a thread's evidence (`commits[]`, `meetings[]`) counts *any* edge, so "PROJ-14 · 7 commits"
...
cite it** — "PROJ-14 is stalled (In Progress, 3 confirmed commits, last moved Jun 25)" with the

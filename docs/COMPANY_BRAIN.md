...
2. **Reference (deterministic parse):** GitHub PR/commit/branch text ("fixes SCRUM-3", "closes #42"), Jira dev-panel links → `jira_issue ↔ github_pr`, `commit ↔ issue`.
...
- **F — Reasoning layer + deterministic commit content (CEO accountability):** ✅ Between a meeting and the work it spawned, the brain now stores a **reasoning node** carrying an alignment **verdict** (`aligned | partial | divergent | unrelated`) so a CEO can see at a glance whether a developer's pushed code matches what was discussed — and divergent/unrelated work is flagged in red.
  - **Deterministic commit content (the "v1/v2" problem):** commit *messages* are untrustworthy fluff, so we don't reason about them. `connectors/github/enrich.ts enrichGithubCommits()` fetches each commit by SHA (`get_commit` — note: passing `detail:true` 400s as "not valid JSON", so we omit it and use `files[]`+`patch`+`stats`), reads the **actual diff**, and a Gemini-flash pass summarizes what truly changed into a separate `knowledge_item.enriched_summary` column (kept apart from `body` so a connector re-sync can't clobber it; commits are immutable → enriched exactly once). Embedding, linking, and the verdict all read `enriched_summary || body`. Runs as its own `__job:'brain-enrich'` on a 1-h EventBridge rule (`wisprnote-api-brain-enrich`) — split out because enriching inline blew the 60-s Lambda timeout.
  - **Verdict:** `connectors/brainReasoning.ts` (`brain_reasoning(meeting_id, impl_id, verdict, rationale, tags)`); `llmLinkMeeting` now returns `{id, relation, verdict, rationale}` — the LLM judges the meeting's intent against the impl item's *real* (diff-grounded) content, and on link writes both a `brain_edge` and a `brain_reasoning` row.
  - **Reasoning lives ON the edge (not a node):** `verdict`+`rationale` are columns on `brain_edge` (`insertEdge` upserts them so re-judging refreshes the line). `GET /brain/graph` returns edges carrying verdict/rationale (backfilled from the `brain_reasoning` ledger so already-judged links colour immediately); `BrainMapModal` colours each LINE by verdict (green aligned / amber partial / red divergent, dashed for divergent), thickens reasoned lines, labels meeting/Jira anchor nodes, and shows the rationale in a panel when a line is **clicked**. The old synthetic `kind:'reasoning'` nodes were removed (they floated disconnected).
  - **Lineage `meeting → Jira task → GitHub commit`:** `judgeAlignment(intent, candidates)` treats BOTH meetings and Jira tasks as intents. A Jira task's candidates are restricted to `['github']` (Turbopuffer `source In` filter), forming the `jira → github` middle hop = "does this commit implement this task?", read from the real diff (Tier 2). Tasks are processed first each tick (few; one-time) so the full chain forms. **Candidate-gen fix:** the ANN must filter by source — meetings cluster so tightly (cosine 0.88–0.99) they otherwise fill every top-K slot and crowd out commits; `queryNearestItems(…, sources)` searches the right pool directly (`CAND_MIN_SIM=0.35`, recall-favouring).
  - **Cost architecture (lazy + tiered — the input-token firewall):** sending every commit's diff to an LLM scales with *commit history × repos × workspaces* and is ruinous. So the diff→LLM step is gated behind a cost firewall — expensive work happens **once, lazily, only for items that get reasoned about**; everything downstream reads cheap compact text.
    - **Tier 0 (ingest, ~$0):** `connectors/github/index.ts` drops merge commits (>1 parent — no own diff). `enrich.ts GENERATED` regex excludes lockfiles/`dist`/vendored/binaries from fingerprint + diff.
    - **Tier 1 (fingerprint, cheap, ALL commits):** `enrich.ts fingerprintCommits()` calls `get_commit` (a GitHub API call — **$0 in LLM tokens**) and stores a deterministic **fingerprint** (filenames + dirs + stats + message line) in `knowledge_item.fingerprint`. `embed.ts` embeds `enriched_summary || fingerprint || body` — so filenames give strong candidate-matching signal with **no diff-LLM pass**. Runs as the `brain-enrich` job (renamed role; 20/tick).
    - **Tier 2 (diff summary, LAZY, candidates only):** `enrich.ts enrichCommit(id)` is called by `brainLink` **only** for the ≤K commits that became a meeting's link candidates → cheap **Gemini Flash** summarizes the real diff into `enriched_summary` (cached forever; commits immutable). The expensive pass now fires for *(meetings × ~6)*, not all commits.
    - **Tier 3 (verdict, quality-first, batched + cached):** `brainLink.llmVerdict` makes **one** batched call per meeting over its top-K candidates on **Sonnet 4.6 → Gemini 3.1 Pro** (`MODELS.brainVerdict`, cross-provider fallback — NOT Opus). Verdict cached in `brain_reasoning` (immutable → compute once).
    - **Candidate generation** (`brainLink` meeting branch): embed the meeting → `queryNearestItems` top-K (floor `CAND_MIN_SIM=0.55`, recall-favouring) → the strict verdict prunes. Per-tick caps (`LLM_MEETINGS_PER_TICK=6`, `CAND_K=6`) bound spend; deferred meetings link on a later tick. **Net:** cost scales with *number of meetings*, not commit volume.
  - **Verified:** meeting "Project Task Alignment" → `NMC-IC-Demo@73c6ba2` **[aligned]** ("tasked Ronika with exploring containerized environments… which this commit does") and → `wisprnote-AI@0a79b97` **[aligned]** ("build a Second Brain using Open Cloud — directly aligns"). The rationales cite real diff content, proving enrichment fed the judgment. brain-link (30 min) + brain-enrich (1 h) crons keep populating verdicts across remaining meetings/commits. *Caveat:* verdict quality tracks diff richness; commits whose `get_commit` fails or yield an empty summary are skipped (marked `enriched_summary=''`).

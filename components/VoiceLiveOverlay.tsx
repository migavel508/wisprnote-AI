  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/95 flex flex-col items-center justify-between p-6 backdrop-blur-md">
      <div className="w-full flex justify-between items-center">
        ...
        <button onClick={() => onClose(transcription)} className="p-2 bg-slate-800 rounded-full hover:bg-slate-700">
          <Icon name="back" className="w-6 h-6" />
        </button>
      </div>

      <div className="flex flex-col items-center justify-center flex-1 w-full space-y-12">
        <div className={`relative w-32 h-32 flex items-center justify-center ${isListening ? 'animate-pulse' : ''}`}>
        ...
        <div className="w-full max-w-md bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6 h-64 overflow-y-auto no-scrollbar space-y-4">
           {isConnecting && <p className="text-slate-400 animate-pulse text-center">Initializing Gemini Live...</p>}
           {!isConnecting && transcription.length === 0 && <p className="text-slate-400 text-center">Start speaking, I'm listening...</p>}
           
           {transcription && (
             <div className="space-y-2">
                <p className="text-xs font-semibold text-indigo-400 uppercase tracking-widest">You</p>
           ...
           {aiResponse && (
             <div className="space-y-2 pt-4 border-t border-slate-700/50">
                <p className="text-xs font-semibold text-emerald-400 uppercase tracking-widest">Lumina AI</p>
      ...
      <button 
        ...
        className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-4 rounded-xl font-bold shadow-xl shadow-indigo-900/40 transition-all transform active:scale-95"
      >
        Finish & Save Transcription
      </button>
    </div>
  );
};

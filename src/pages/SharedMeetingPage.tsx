...
const FORCE_LIGHT: React.CSSProperties = {
  ...
  '--color-app-canvas': '#f5f5f5',
  '--color-app-panel': '#ffffff',
  '--color-app-raised': '#f2f2f2',
  '--color-app-chip': '#ffffff',
  '--color-app-border': 'rgba(0, 0, 0, 0.08)',
  '--color-app-border-strong': '#e0e0e0',
  '--color-app-card-border': 'rgba(0, 0, 0, 0.06)',
  '--color-app-fg': '#1a1a1a',
  '--color-app-fg-muted': '#666666',
  '--color-app-fg-subtle': '#888888',
  '--color-app-fg-label': '#a0a0a0',
  '--color-app-divider': 'rgba(0, 0, 0, 0.07)',
  color: '#1a1a1a',
...
function BrandMark({ size = 'md' }: { size?: 'sm' | 'md' }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`${textSize} font-bold tracking-[0.02em] text-[#1a1a1a]`} style={{ fontFamily: BRAND_FONT }}>
        Wisprnote AI
      </span>
    </div>
  );
...
function PoweredBy({ className = '' }: { className?: string }) {
  return (
    <div className={`flex items-center justify-center gap-1.5 ${className}`}>
      <span className="text-[11px] text-[#aaaaaa]">Powered by</span>
      <span className="text-[11px] font-bold text-[#222222]" style={{ fontFamily: BRAND_FONT }}>Wisprnote AI</span>
...
export default function SharedMeetingPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={FORCE_LIGHT}>
        <div className="text-center">
          ...
          <p className="text-[13px] text-[#aaaaaa]">Loading shared meeting…</p>
        </div>
      </div>
    );
  }
  ...
  if (state.status === 'sign_in') {
    return (
      <div className="min-h-screen flex flex-col" style={FORCE_LIGHT}>
        <header className="px-6 py-4 border-b border-[#e0e0e0]/60"><BrandMark /></header>
        <div className="flex-1 flex items-center justify-center px-4">
          <div className="bg-white rounded-2xl shadow-sm p-8 w-full max-w-sm border border-[#e0e0e0]/80">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-xl bg-amber-50 flex items-center justify-center">
                <Lock className="w-5 h-5 text-amber-500" />
              </div>
              <div>
                <h1 className="text-[15px] font-semibold text-[#1a1a1a]">Sign in to view</h1>
            ...
            <form onSubmit={handleSignIn} className="space-y-3">
              <input type="email" placeholder="Email address" value={signInEmail} onChange={e => setSignInEmail(e.target.value)} required
                className="w-full px-4 py-2.5 text-[13px] bg-[#f5f5f5] rounded-xl border-none outline-none text-[#1a1a1a] placeholder:text-[#b8b8b8] focus:ring-2 focus:ring-[#d0c8bf]/50" />
            ...
            <PoweredBy className="mt-6 pt-4 border-t border-[#e0e0e0]/60" />
          </div>
        </div>
      </div>
    );
  ...
  if (state.status === 'denied') {
    return (
      <div className="min-h-screen flex flex-col" style={FORCE_LIGHT}>
        <header className="px-6 py-4 border-b border-[#e0e0e0]/60"><BrandMark /></header>
        <div className="flex-1 flex items-center justify-center px-4">
          <div className="bg-white rounded-2xl shadow-sm p-8 w-full max-w-sm border border-[#e0e0e0]/80 text-center">
  ...
  const prose = `prose prose-sm max-w-none
    prose-headings:text-[#1a1a1a] prose-headings:tracking-tight prose-headings:font-semibold
    prose-h2:text-[15px] prose-h3:text-[14px]
    prose-p:text-[14px] prose-p:leading-[1.8] prose-p:text-[#222222]
    prose-li:text-[14px] prose-li:text-[#222222] prose-li:leading-[1.7]
    prose-strong:text-[#1a1a1a] prose-strong:font-semibold
    prose-blockquote:border-l-[#e8e2da] prose-blockquote:text-[#7a6d62]
    prose-td:text-[#222222] prose-th:text-[#1a1a1a]
    markdown-body`;
  ...
  return (
    <div className="min-h-screen flex flex-col" style={FORCE_LIGHT}>
      ...
      <header className="bg-white border-b border-[#e0e0e0]/80 sticky top-0 z-20 flex-none">
        <div className="max-w-screen-xl mx-auto px-5 sm:px-8 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="hidden sm:inline text-[13px] text-[#888888] hover:text-[#1a1a1a] cursor-pointer transition-colors">Sign in</span>
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 flex max-w-screen-xl mx-auto w-full">
        ...
        <main className="flex-1 min-w-0 overflow-y-auto">
          <div className="max-w-2xl mx-auto px-5 sm:px-10 pt-12 sm:pt-16 pb-28">
            ...
            {/* Title */}
            <h1 className="text-[30px] sm:text-[40px] font-bold tracking-[-0.025em] text-[#1a1a1a] leading-[1.12] mb-5"
                ...
                <button key={t} onClick={() => setTab(t)}
                  className={`px-4 py-1.5 rounded-full text-[12px] font-medium transition-all border ${
                    tab === t
                      ...
                      : 'text-[#888888] border-[#e8e2da] hover:text-[#5c5147] hover:border-[#d0c8bf]'
                  }`}>
        ...
        {canChat && (
          <aside className="hidden md:flex flex-col w-[280px] lg:w-[320px] flex-none border-l border-[#e0e0e0] bg-white sticky top-[49px] h-[calc(100vh-49px)]">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 min-h-0">
                ...
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[90%] px-3.5 py-2.5 rounded-2xl text-[13px] leading-relaxed ${
                    msg.role === 'user'
                      ...
                      : 'bg-[#f0f0f0] text-[#222222] rounded-tl-sm'
                  }`}>
                    {msg.role === 'model' ? (
                      <div className="prose prose-sm max-w-none prose-p:text-[13px] prose-p:leading-relaxed prose-p:text-[#222222] prose-strong:text-[#1a1a1a] prose-li:text-[#222222] markdown-body" style={{ color: '#2c2520' }}>
              ...
              {isSending && (
                <div className="flex justify-start">
                  <div className="px-3.5 py-2.5 bg-[#f0f0f0] rounded-2xl rounded-tl-sm">
            ...
            <div className="flex-none px-4 pb-4 pt-2">
              <div className="flex items-center gap-2 border border-[#e8e2da] rounded-xl px-3.5 py-2.5 bg-white hover:border-[#d0c8bf] focus-within:border-[#c4bab0] focus-within:ring-1 focus-within:ring-[#e8e2da] transition-all">
                <input ref={inputRef} type="text"
                  ...
                  className="flex-1 bg-transparent border-none outline-none text-[13px] text-[#1a1a1a] placeholder:text-[#b8b8b8] disabled:opacity-50" />
                <div className="flex items-center gap-0.5 flex-shrink-0">
                  <button className="p-1.5 text-[#b8b8b8] hover:text-[#888888] rounded-lg hover:bg-[#f5f5f5] transition-all" title="Attach">
                    <Paperclip className="w-[15px] h-[15px]" />
                  </button>
                  <button className="p-1.5 text-[#b8b8b8] hover:text-[#888888] rounded-lg hover:bg-[#f5f5f5] transition-all" title="Voice">
      ...
      {canChat && !mobileChatOpen && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-30 md:hidden">
          <button onClick={() => { setMobileChatOpen(true); setTimeout(() => inputRef.current?.focus(), 150); }}
            className="flex items-center gap-2 px-5 py-2.5 bg-white text-[#1a1a1a] rounded-full shadow-lg border border-[#e0e0e0] hover:shadow-xl hover:border-[#d0c8bf] transition-all text-[13px] font-medium">
      ...
      <AnimatePresence>
        {mobileChatOpen && (
          <motion.div
            ...
            className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-[#e0e0e0] shadow-2xl rounded-t-2xl flex flex-col"
            ...
            <div className="flex-none pt-2 pb-0">
              ...
              <div className="flex items-center justify-between px-5 py-2 border-b border-[#e0e0e0]">
                <BrandMark size="sm" />
                <div className="flex items-center gap-2.5">
                  <span className="text-[10px] text-[#aaaaaa]">{remaining} left</span>
                  <button onClick={() => setMobileChatOpen(false)}
                    className="w-6 h-6 rounded-full bg-[#f0f0f0] flex items-center justify-center text-[#888888] hover:bg-[#e5e5e5] transition-colors">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 min-h-[80px]">
              {chatMessages.length === 0 && (
                <p className="text-[13px] text-[#aaaaaa] text-center py-6">Ask anything about this meeting…</p>
              )}
              {chatMessages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] px-3.5 py-2.5 rounded-2xl text-[13px] leading-relaxed ${
                    msg.role === 'user' ? 'bg-[#1c1917] text-white rounded-tr-sm' : 'bg-[#f0f0f0] text-[#222222] rounded-tl-sm'
                  }`}>
                    {msg.role === 'model' ? (
                      <div className="prose prose-sm max-w-none prose-p:text-[13px] prose-p:leading-relaxed prose-p:text-[#222222] prose-strong:text-[#1a1a1a] prose-li:text-[#222222] markdown-body" style={{ color: '#2c2520' }}>
                        <Markdown remarkPlugins={[remarkGfm]}>{msg.text}</Markdown>
            ...
            {/* Input */}
            <div className="flex-none px-4 py-3 border-t border-[#e0e0e0]">
              <div className="flex items-center gap-2 border border-[#e8e2da] rounded-xl px-3.5 py-2.5">
                <input type="text" placeholder={remaining > 0 ? 'Ask anything' : 'Limit reached'}
                  ...
                  className="flex-1 bg-transparent border-none outline-none text-[13px] text-[#1a1a1a] placeholder:text-[#b8b8b8] disabled:opacity-50" />
                <div className="flex items-center gap-0.5 flex-shrink-0">
                  <button className="p-1.5 text-[#b8b8b8] hover:text-[#888888] rounded-lg hover:bg-[#f5f5f5] transition-all">
                    <Paperclip className="w-[15px] h-[15px]" />
                  </button>
                  <button className="p-1.5 text-[#b8b8b8] hover:text-[#888888] rounded-lg hover:bg-[#f5f5f5] transition-all">
                    <Mic className="w-[15px] h-[15px]" />
      ...
      {/* ── Footer ── */}
      <footer className="border-t border-[#e0e0e0]/60 bg-white py-5">

...
function BrandMark({ size = 'md', onDark = false }: { size?: 'sm' | 'md'; onDark?: boolean }) {
  const imgSize = size === 'sm' ? 'w-5 h-5' : 'w-7 h-7';
  const textSize = size === 'sm' ? 'text-[13px]' : 'text-[16px]';
  const textColor = onDark ? 'text-white' : 'text-[#1c1917]';
  return (
    <div className="flex items-center gap-2">
      <img src="/logo_status.png" alt="Wisprnote AI" className={`${imgSize} rounded-lg object-cover`} />
      <span className={`${textSize} font-bold tracking-[-0.01em] ${textColor}`} style={{ fontFamily: BRAND_FONT }}>
        Wisprnote AI
      </span>
    </div>
  );
...
export default function SharedMeetingPage() {
  ...
  if (state.status === 'sign_in') {
    return (
      <div className="min-h-screen bg-[#faf8f5] flex flex-col">
        <header className="px-6 py-4"><BrandMark /></header>
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5] flex flex-col" style={{ colorScheme: 'light' }}>
        ...
        <div className="max-w-screen-xl mx-auto px-5 sm:px-8 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a href="https://wisprnote.com" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1.5 px-4 py-1.5 bg-white text-[#1c1917] text-[12px] font-semibold rounded-full hover:bg-gray-100 transition-colors">
            ...
            <span className="hidden sm:inline text-[13px] text-white/60 hover:text-white cursor-pointer transition-colors">Sign in</span>
          </div>
        </div>
      </header>
              ...
              {chatMessages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[90%] px-3.5 py-2.5 rounded-2xl text-[13px] leading-relaxed ${
                    ...
                    {msg.role === 'model' ? (
                      <div className="prose prose-sm max-w-none prose-p:text-[13px] prose-p:leading-relaxed prose-p:text-[#2c2520] prose-strong:text-[#1c1917] prose-li:text-[#2c2520] markdown-body">
      ...
      {/* ── Floating "Chat with AI" pill ── */}
      {canChat && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-30">
          <button
            onClick={() => { setMobileChatOpen(true); setTimeout(() => inputRef.current?.focus(), 150); }}
            className="md:hidden flex items-center gap-2 px-5 py-2.5 bg-white text-[#1c1917] rounded-full shadow-lg border border-[#ece6de] hover:shadow-xl hover:border-[#d0c8bf] transition-all text-[13px] font-medium">
      ...
      {/* ── Footer ── */}
      <footer className="border-t border-[#ece6de]/60 bg-[#faf8f5] py-5">
